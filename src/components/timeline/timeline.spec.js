...
describe('TimelineComponent', () => {
  ...
  describe('Unit tests', () => {
    it('should get icon to EVALUATION card', () => {
      expect(
        getCardIcon({ ...timelineItemMock, cardType: 'EVALUATION' }),
      ).toEqual(faUsers);
    });
    ...
    it('should get icon to ACCOUNTABILITY_CREATED card', () => {
      expect(
        getCardIcon({ ...timelineItemMock, cardType: 'ACCOUNTABILITY_CREATED' }),
      ).toEqual(faAsterisk);
    });
    ...
    it('should get invalid icon to EXPENSE card', () => {
      expect(
        getCardIcon({
          ...timelineItemMock,
          cardType: 'EXPENSE',
          expenseTypeIcon: 'invalid',
        }),
      ).toBeNull();
    });

    it('should get null icon to ivalid card type', () => {
      expect(getCardIcon('invalid')).toBeNull();
    });
  ...
  describe('Snapshot tests', () => {
    it('should render timeline', async () => {
      requestTimelineData.mockImplementation(() => Promise.resolve([timelineItemMock]));
      ...
      expect(pretty(container.innerHTML)).toMatchInlineSnapshot(`
        "<div class=\\"MuiPaper-root MuiCard-root MuiPaper-elevation1 MuiPaper-rounded\\" style=\\"margin-top: 24px; padding: 16px;\\">
          ...
          <div><span>TYPE</span><span>Hotel</span></div>
          <div><span>VALOR</span><span>11.11</span><span>222.22</span></div>
          <div><span>STATUS</span><span>Pending</span><span>sadasd</span></div>
          <div><svg aria-hidden=\\"true\\" focusable=\\"false\\" data-prefix=\\"fas\\" data-icon=\\"receipt\\" class=\\"svg-inline--fa fa-receipt fa-w-12 \\" role=\\"img\\" xmlns=\\"http://www.w3.org/2000/svg\\" viewBox=\\"0 0 384 512\\">
            ...
            </svg><a href=\\"https://staging-backoffice-hotmart.s3.amazonaws.com/accountability/collaborator_6393359/accountability_742/expense_545_file_615_Captura%20de%20tela%20de%202019-11-24%2010-15-50.png\\">Receipt</a></div>
        </div>"
      `);
    });
  });
